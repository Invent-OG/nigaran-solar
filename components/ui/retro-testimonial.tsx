    ...
    <>
        ...
        {isExpanded && (
          <div className="fixed inset-0 h-screen overflow-hidden z-50">
            ...
            <motion.div
              ...
              className="max-w-5xl mx-auto bg-gradient-to-b from-primary to-primary/40 h-full z-[60] p-4 md:p-10 rounded-3xl relative md:mt-10"
            >
              <button
              ...
              <motion.p
                ...
                className="px-0 md:px-20  text-lg dark:text-white   underline underline-offset-8"
              >
                {testimonial.designation}
              </motion.p>
              <motion.p
                layoutId={layout ? `title-${testimonial.name}` : undefined}
                className="px-0 md:px-20 text-2xl md:text-4xl font-normal italic  mt-4 dark:text-white  lowercase"
              >
                {testimonial.name}
              </motion.p>
              <div className="py-8  px-0 md:px-20 text-3xl lowercase    tracking-wide">
                <Quote className="h-6 w-6  dark:text-neutral-900" />
      ...
      >
        <div
          ...
          } rounded-3xl  h-[500px] md:h-[550px] w-80 md:w-96 overflow-hidden flex flex-col items-center justify-center relative z-10 shadow-md`}
        >
          <div className="absolute opacity-50 " style={{ inset: "-1px 0 0" }}>
          ...
          <motion.p
            layoutId={layout ? `title-${testimonial.name}` : undefined}
            className=" text-2xl md:text-2xl font-normal  text-center [text-wrap:balance]   mt-4 lowercase px-3"
          >
          ...
          <motion.p
            layoutId={layout ? `category-${testimonial.name}` : undefined}
            className=" text-xl md:text-2xl font-thin   text-center mt-5 lowercase"
          >
            {testimonial.name}.
          </motion.p>
          <motion.p
            layoutId={layout ? `category-${testimonial.name}` : undefined}
            className=" text-base md:text-base font-thin   text-center mt-1 lowercase underline underline-offset-8 decoration-1"
          >
...
const ProfileImage = ({ src, alt, ...rest }: ImageProps) => {
  ...
  return (
    <div className="w-[90px] h-[90px] md:w-[150px] md:h-[150px]  overflow-hidden rounded-[1000px] border-[3px] border-solid  aspect-[1/1] flex-none  relative">
      <Image
        className={cn(
